...
import { ECOMMERCE\_PLATFORMS, SKU\_STYLES, COLORS, SIZES } from '../constants';
import { Wand2Icon, RefreshCwIcon, ImageIcon, DollarSignIcon, ChevronDownIcon, SparklesIcon } from './Icons';
...
platform: ECOMMERCE\_PLATFORMS\[0].name,
skuStyle: SKU\_STYLES\[0].name,
colors: \[],
sizes: \[],
});
const \[imagePreview, setImagePreview] = useState\<string | null>(null);

const activeSkuStyle = SKU\_STYLES.find(s => s.name === formData.skuStyle);
...
// Keep preview in sync with typed Image URL
useEffect(() => {
...
return ( <form onSubmit={handleSubmit} className="bg-gray-900/70 border border-green-500/30 rounded-lg"> <div className="p-6 space-y-6"> <div> <h3 className="text-lg font-semibold text-gray-200 mb-2">1. Select Platform & SKU Style</h3> <div className="flex items-center border-b border-green-500/30 mb-4">
{ECOMMERCE\_PLATFORMS.map((p) => (
\<button
key={p.name}
type="button"
...
\>
{p.name}
{formData.platform === p.name && ( <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-green-400"></div>
)} </button>
))} </div> <div> <label htmlFor="skuStyle" className="block text-sm font-medium text-green-300/80 mb-1">SKU Style</label> <select name="skuStyle" id="skuStyle" value={formData.skuStyle} onChange={handleInputChange} className="w-full bg-gray-950/50 border border-gray-700 rounded-md py-2 px-3 text-gray-200">
{SKU\_STYLES.map(style => <option key={style.name} value={style.name}>{style.name}</option>)} </select> <p className="text-xs text-gray-500 mt-1 font-mono">{activeSkuStyle?.description}</p> </div> </div>
...
```
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <InputField id="productUrl" label="Product URL (Optional)" value={formData.productUrl || ''} onChange={handleInputChange} placeholder="https://..." type="url" />
        <InputField id="imageUrl" label="Image URL (Optional)" value={formData.imageUrl || ''} onChange={handleInputChange} placeholder="https://.../image.jpg" type="url" />
